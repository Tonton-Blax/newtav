Date.prototype.getWeek = function(){
  ...
  export const API = 'https://www.taverneasy.fr/wordpress/';
  /*
  ...
  */
  ...
  export const EVENTS = 'wp-json/geodir/v2/events';
  export const ASSOS = 'wp-json/geodir/v2/places'
  export const LIBERALES = 'wp-json/geodir/v2/liberales'
  //const ASSOSCAT = 'wp-json/geodir/v2/places/categories'
  export const COMMERCESCAT = 'wp-json/geodir/v2/commerces/categories'
  export const OPTIONS = `?per_page=100`;
  export const URLFORM = 'http://taverneasy.fr/wp-json/contact-form-7/v1/contact-forms/430/feedback'
    ...
    export async function getWeather () {
      ...
      if (!weather) {
        if (!getWithExpiry('weather')) {
            let blub = await fetch(WEATHERAPI);
            weather = await blub.json();
    ...
    async function getCommerces (options) {
      ...
      if (!commerces) {
        if (!getWithExpiry('localcommerces')) {
          const res = await fetch(`${API}${COMMERCES}${options || OPTIONS}`);
          commerces = res.json();
    ...
    async function getLiberales (options) {
      ...
      if (!liberales) {
        if (!getWithExpiry('liberales')) {
          const res = await fetch(`${API}${LIBERALES}${options || OPTIONS}`);
          liberales = res.json();
    ...
    async function getAssos (options) {
      if (!associations) {
        if (!getWithExpiry('localassos')) {
          const res = await fetch(`${API}${ASSOS}${options || OPTIONS}`);
    ...
    async function getEvents (options) {
      if (!events) {
        if (!getWithExpiry('localevents')) {
          const res = await fetch(`${API}${EVENTS}${options || OPTIONS}`);
          events = res.json();
  ...
  export async function makeCategories (slug) {
    const res = await fetch(`https://www.taverneasy.fr/wordpress/wp-json/geodir/v2/${slug}/categories${OPTIONS}`);
    let h = res.json();
    let r = {};
